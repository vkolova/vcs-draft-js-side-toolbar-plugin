/* eslint-disable react/no-array-index-key */
import React from 'react';
import PropTypes from 'prop-types';

class BlockTypeSelect extends React.Component {
  state = {
    show: false,
    overrideContent: undefined,
    style: {
      transform: 'translate(-50%) scale(0)',
    }
  }

  onMouseEnter = () => {
    this.setState({
      show: true,
      style: {
        transform: 'translate(-50%) scale(1)',
        transition: 'transform 0.15s cubic-bezier(.3,1.2,.2,1)',
      },
    });
  }

  onMouseLeave = () => {
    this.setState({
      show: false,
      style: {
        transform: 'translate(-50%) scale(0)',
      },
    });
  }

  onMouseDown = (clickEvent) => {
    clickEvent.preventDefault();
    clickEvent.stopPropagation();
  }

  onOverrideContent = (overrideContent) => this.setState({ overrideContent });

  toggle = () =>
    this.state.show
      ? this.onMouseLeave()
      : this.onMouseEnter();

  render() {
    const { theme, getEditorState, setEditorState } = this.props;
    const { overrideContent: OverrideContent } = this.state;
    const childrenProps = {
      theme: theme.buttonStyles,
      getEditorState,
      setEditorState,
      onOverrideContent: this.onOverrideContent
    };

    return (
      <div>
        <div className={theme.blockTypeSelectStyles.blockType} onClick={this.toggle}>
          { this.props.wrapperIcon }
        </div>

        {
          OverrideContent
            ? <div className={theme.blockTypeSelectStyles.popup} style={this.state.style}>
              <OverrideContent {...childrenProps} />
            </div>
            : <div className={theme.blockTypeSelectStyles.popup} style={this.state.style} onClick={this.onMouseLeave}>
              { this.props.children({
                getEditorState,
                setEditorState,
                theme: theme.buttonStyles
              }) }
            </div>
        }
      </div>
    );
  }
}

BlockTypeSelect.propTypes = {
  children: PropTypes.func
};

BlockTypeSelect.defaultProps = {
  wrapperIcon: <svg height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
    <path d="M0 0h24v24H0z" fill="none" />
    <path d="M6 10c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm12 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm-6 0c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z" />
  </svg>
};

export default BlockTypeSelect;

